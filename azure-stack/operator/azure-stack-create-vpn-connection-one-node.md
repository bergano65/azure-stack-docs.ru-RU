...
В этой статье объясняется, как создать VPN-подключение типа "сеть — сеть" между двумя виртуальными сетями в двух отдельных средах Пакета средств разработки Azure Stack (ASDK). Во время настройки подключений вы узнаете, как работают VPN-шлюзы Azure Stack.
...
В таблице ниже перечислены параметры сети обеих сред ASDK. Выполните процедуру, приведенную после таблицы, чтобы добавить адрес EBGP, соответствующий вашей сети.
...
Теперь создайте виртуальную машину в POC2, а затем поместите ее в подсеть виртуальной машины в своей виртуальной сети:
...
Среда ASDK работает автономно и изолирована от сети, в которой развернут физический узел. Поэтому *внешняя* сеть VIP, к которой подключены шлюзы, фактически не является внешней. Вместо этого сеть VIP скрыта за маршрутизатором, выполняющим преобразование сетевых адресов.

В качестве маршрутизатора используется виртуальная машина Windows Server с именем **AzS-bgpnat01**, на которой выполняется роль служб маршрутизации и удаленного доступа в инфраструктуре ASDK. NAT необходимо настроить на виртуальной машине AzS-bgpnat01. Это позволит установить VPN-подключение типа "сеть — сеть" на обоих концах.

Чтобы настроить VPN-подключение, необходимо создать маршрут статического сопоставления NAT. Это позволит сопоставить внешний интерфейс на виртуальной машине BGPNAT с виртуальным IP-адресом пула пограничных шлюзов. Маршрут статического сопоставления NAT следует настроить в каждом порте VPN-подключения.
...
3. Вставьте и измените приведенный ниже сценарий PowerShell. Чтобы настроить NAT в каждом Пакете средств разработки Azure Stack, выполните приведенный ниже сценарий в интегрированной среде сценариев Windows PowerShell с повышенными привилегиями. В скрипте добавьте значения в заполнители `External BGPNAT address` и `Internal IP address`:
...
Теперь, когда подключение типа "сеть —сеть" установлено, необходимо проверить, может ли трафик проходить через него. Чтобы выполнить проверку, войдите на одну из виртуальных машин, созданных в любой из сред ASDK. Затем проверьте связь виртуальной машины, созданной в другой среде.
...
10. На виртуальной машине в POC2 проверьте связь с виртуальной машиной в POC1 через туннель. Для этого проверьте связь с прямым IP-адресом, записанным из виртуальной машины VM01. В примере среды этот адрес — **10.0.10.4**, но вам следует использовать адрес, записанный в лаборатории. Должен отобразится результат, как в этом примере:

    ![Успешная проверка связи](media/azure-stack-create-vpn-connection-one-node-tp2/image19b.png)
11. Ответ из удаленной виртуальной машины указывает, что проверка выполнена успешно. Вы можете закрыть окно виртуальной машины. Чтобы проверить подключение, вы можете выполнить другие операции передачи данных, например копирование файлов.
