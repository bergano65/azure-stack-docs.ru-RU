---
title: Интеграция DNS центра обработки данных в Azure Stack | Документация Майкрософт
...
ms.locfileid: "72780492"
---
# <a name="azure-stack-datacenter-dns-integration"></a>Интеграция DNS центра обработки данных Azure Stack

Чтобы иметь возможность доступа к конечным точкам Azure Stack (**portal**, **adminportal**, **management** и **adminmanagement**) извне Azure Stack, необходимо интегрировать службы DNS Azure Stack с DNS-серверами, на которых размещаются зоны DNS, которые нужно использовать в Azure Stack.
...
|DNS-серверы пересылки|DNS-серверы, которые используются для перенаправления запросов DNS, зон и записей DNS, размещенных за пределами Azure Stack, либо в корпоративной интрасети, либо в общедоступном Интернете. После развертывания значение DNS-сервера пересылки можно изменить с помощью командлета [**Set-AzSDnsForwarder** ](#editing-dns-forwarder-ips). 
...
IP-адреса DNS-сервера пересылки устанавливаются во время развертывания Azure Stack. Однако, если по какой-либо причине требуется обновление IP-адресов сервера пересылки, вы можете изменить значения, подключившись к привилегированной конечной точке и выполнив командлеты PowerShell `Get-AzSDnsForwarder` и `Set-AzSDnsForwarder [[-IPAddress] <IPAddress[]>]`. Дополнительные сведения см. в статье [Использование привилегированной конечной точки в Azure Stack](azure-stack-privileged-endpoint.md).
