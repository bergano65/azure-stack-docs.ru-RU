...
3. В колонке **Настраиваемое развертывание** выберите **Изменить шаблон** > **Шаблон быстрого запуска**, а затем используйте раскрывающийся список доступных шаблонов, чтобы выбрать шаблон **appservice-fileshare-sqlserver-ha**, а потом нажмите кнопку **ОК** и щелкните **Сохранить**.
...
5. В колонке **Настраиваемое развертывание** выберите значение **Default Provider Subscription** (Подписка поставщика по умолчанию) в качестве используемой подписки. Затем создайте группу ресурсов или выберите имеющуюся группу ресурсов для настраиваемого развертывания.<br><br> Затем выберите расположение группы ресурсов (**локальное** для установки ASDK) и нажмите кнопку **Создать**. Перед началом развертывания шаблона произойдет проверка параметров настраиваемого развертывания.

    ![Создание настраиваемого развертывания](media/app-service-deploy-ha/4.png)

6. На портале администрирования выберите **Группы ресурсов**, а затем имя группы ресурсов, созданной для настраиваемого развертывания (в этом примере **app-service-ha**). Просмотрите состояние развертывания, чтобы убедиться, что все развертывания выполнены успешно.

   > [!NOTE]
   > Развертывание шаблона займет около часа.
...
После успешного развертывания шаблона запишите его выходные данные. Вам нужно будет предоставить эти сведения при запуске установщика Службы приложений. 
...
2. На портале администрирования выберите **Группы ресурсов**, а затем имя группы ресурсов, созданной для настраиваемого развертывания (в этом примере **app-service-ha**). 

3. Щелкните **Развертывания** и выберите **Microsoft.Template**.

    ![Развертывание Microsoft.Template](media/app-service-deploy-ha/6.png)

4. После выбора развертывания **Microsoft.Template** щелкните **Выходные данные** и запишите выходные значения параметров шаблона. Эти сведения потребуются при развертывании Службы приложений.
...
> [!TIP]
> Не все шаги, описанные в статье о начале работы, являются обязательными, так как при развертывании шаблона виртуальные машины инфраструктуры настраиваются с учетом ваших требований. 
...
- Создайте приложение удостоверений на основе поставщика удостоверений, выбранного для Azure Stack. Приложения удостоверений можно создать для [Azure AD](azure-stack-app-service-before-you-get-started.md#create-an-azure-active-directory-application) или [служб федерации Active Directory](azure-stack-app-service-before-you-get-started.md#create-an-active-directory-federation-services-application). После создания запишите идентификатор приложения.
- Убедитесь, что образ Windows Server 2016 Datacenter добавлен в Azure Stack Marketplace. Это необходимо для установки Службы приложений.

### <a name="deploy-app-service-in-highly-available-configuration"></a>Развертывание Службы приложений в конфигурации с высоким уровнем доступности
...
2. Выберите **Развернуть службу приложений или провести обновление до последней версии**.
    ...
    ![Развертывание или обновление](media/app-service-deploy-ha/01.png)

3. Примите условия лицензирования корпорации Майкрософт и щелкните **Далее**.

    ![Условия лицензирования корпорации Майкрософт](media/app-service-deploy-ha/02.png)

4. Примите условия лицензирования сторонних разработчиков и щелкните **Далее**.

    ![Условия лицензирования сторонних разработчиков](media/app-service-deploy-ha/03.png)

5. Укажите конфигурацию облачной конечной точки Службы приложений для среды Azure Stack.

    ![Конфигурация облачной конечной точки Службы приложений](media/app-service-deploy-ha/04.png)

6. **Подключитесь** к подписке Azure Stack, которая будет использоваться для установки, и выберите расположение. 

    ![Подключение к подписке Azure Stack](media/app-service-deploy-ha/05.png)
...
7. Установите переключатель **Использовать существующие виртуальные сети и подсети** и укажите **имя группы ресурсов**, используемой для развертывания шаблона с высоким уровнем доступности.<br><br>Затем выберите виртуальную сеть, созданную как часть развертывания шаблона, и соответствующие подсети ролей из вариантов в раскрывающихся списках. 
    ...
    ![Выбор виртуальной сети](media/app-service-deploy-ha/06.png)
...
8. Предоставьте ранее записанные выходные данные шаблона, в частности путь к общей папке и параметры владельца общей папки. По завершении нажмите кнопку **Далее**.
    ...
    ![Выходные значения для общей папки](media/app-service-deploy-ha/07.png)

9. Так как компьютер, на котором устанавливается Служба приложений, и файловый сервер, на котором размещена общая папка Службы приложений, находятся в разных виртуальных сетях, вы не сможете разрешить имя. **Это ожидаемое поведение**.<br><br>Убедитесь, что указанные сведения о UNC-пути к общей папке и об учетных записях верны, и нажмите кнопку **Да** в диалоговом окне уведомления, чтобы продолжить установку Службы приложений.

    ![Диалоговое окно ожидаемой ошибки](media/app-service-deploy-ha/08.png)
...
10. Укажите идентификатор приложения удостоверений, а также пути и пароли для сертификатов удостоверений и щелкните **Далее**:
    ...
    ![Сертификат приложения удостоверений и корневой сертификат](media/app-service-deploy-ha/008.png)

10. Затем предоставьте остальные необходимые сведения для следующих сертификатов и щелкните **Далее**:
    ...
    ![Дополнительные сертификаты конфигурации](media/app-service-deploy-ha/09.png)

11. Укажите сведения о подключении к SQL Server, используя сведения о подключении из выходных данных развертывания шаблона с высоким уровнем доступности:

    ![Сведения о подключении к SQL Server](media/app-service-deploy-ha/10.png)

12. Так как компьютер, на котором устанавливается Служба приложений, и сервер SQL Server, на котором размещены базы данных Службы приложений, находятся в разных виртуальных сетях, вы не сможете разрешить имя.  **Это ожидаемое поведение**.<br><br>Убедитесь, что значение, указанное в качестве имени SQL Server, и сведения об учетных записях верны, и нажмите кнопку **Да**, чтобы продолжить установку Службы приложений. Щелкните **Далее**.

    ![Сведения о подключении к SQL Server](media/app-service-deploy-ha/11.png)

13. Примите значения конфигурации роли по умолчанию или измените их, а затем щелкните **Далее**.<br><br>Мы рекомендуем изменить значения по умолчанию для экземпляров роли инфраструктуры Службы приложений следующим образом:
    ...
    ![Значения экземпляра роли инфраструктуры](media/app-service-deploy-ha/12.png)
...
14. Выберите образ платформы, который будет использоваться для установки виртуальных машин инфраструктуры Службы приложений, и щелкните **Далее**:

    ![Выбор образа платформы](media/app-service-deploy-ha/13.png)

15. Укажите учетные данные роли инфраструктуры Службы приложений и щелкните **Далее**:

    ![Учетные данные роли инфраструктуры](media/app-service-deploy-ha/14.png)

16. Просмотрите сведения, которые будут использоваться для развертывания Службы приложений, и щелкните **Далее**, чтобы начать развертывание. 

    ![Просмотр сводки установки](media/app-service-deploy-ha/15.png)

17. Следите за ходом выполнения развертывания Службы приложений. Процесс может занять более часа в зависимости от конкретной конфигурации развертывания и оборудования. После успешной установки выберите **Выйти**.
...
[Создание резервной копии Службы приложений](app-service-back-up.md) После успешного развертывания и настройки Службы приложений следует создать резервную копию всех компонентов, необходимых для аварийного восстановления, чтобы предотвратить потерю данных и избежать простоев во время операций восстановления.