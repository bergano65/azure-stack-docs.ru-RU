...
>[!Note]
>В качестве альтернативы сбору журналов по запросу можно упростить процесс устранения неполадок, включив [автоматический сбор журналов диагностики](azure-stack-configure-automatic-diagnostic-log-collection-tzl.md). Если необходимо исследовать состояние работоспособности системы, журналы могут автоматически передаваться для анализа с помощью CSS. 
