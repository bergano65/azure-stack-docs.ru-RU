...
Для обновления Azure Stack Hub 1906 используется тип сборки **Экспресс**. См. сведения о типах сборки обновления в руководстве по [управлению обновлениями в Azure Stack Hub](azure-stack-updates.md). Установка обновления 1906 составляет около 10 часов независимо от количества физических узлов в среде Azure Stack Hub. Точное время обновления обычно зависит от емкости, используемой рабочими нагрузками клиента в системе, сетевого подключения (при подключении к Интернету) и характеристик оборудования системы. Более длительное обновление также следует считать нормальным, и операторам Azure Stack Hub не нужно предпринимать в этом случае никаких действий (если обновление не завершится ошибкой). Это приблизительное значение относится только к обновлению 1906, и нет смысла сопоставлять его с другими обновлениями Azure Stack Hub.
...
- Командлет **Get-GraphApplication** в привилегированной конечной точке теперь отображает отпечаток сертификата, использующегося в текущий момент. Это обновление оптимизирует управление сертификатами субъектов-служб при развертывании Azure Stack Hub с помощью AD FS.
...
- В выпуске 1906 доступно более подробное представление о ходе выполнения обновления, благодаря чему можно легко проверить, не приостановлены ли обновления. В результате увеличено общее количество этапов обновления, которые операторы видят в колонке **Обновление**. Вы можете также заметить, что по сравнению с предыдущими обновлениями, теперь в параллельном режиме выполняется больше процедур обновления.
...
- На порталах администратора и пользователя исправлена проблема, из-за которой в Marketplace при поиске некорректно возвращалось расширение Docker, с которым нельзя выполнить никакие действия, так как это расширение не доступно в Azure Stack Hub.

- Устранена проблема в пользовательском интерфейсе развертывания шаблона, из-за которой параметры не заполнялись, если имя шаблона начиналось с символа подчеркивания ("_").
