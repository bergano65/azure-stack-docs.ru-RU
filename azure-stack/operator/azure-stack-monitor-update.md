...
[Привилегированную конечную точку](azure-stack-privileged-endpoint.md) можно использовать для отслеживания хода выполнения обновления Azure Stack. Если портал Azure Stack станет недоступным, привилегированная конечная точка позволит возобновить обновление, завершившееся сбоем, с последнего успешного шага. Использование портала Azure Stack — рекомендуемый метод для управления обновлениями в Azure Stack.
...
1. Создайте сеанс PowerShell на любой из виртуальных машин (ВМ) ERCS в среде Azure Stack (*префикс*-ERCS01, *префикс*-ERCS02 или *префикс*-ERCS03). Замените *префикс* строкой префикса ВМ, которая соответствует используемой среде.
