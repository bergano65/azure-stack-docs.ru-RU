---
title: Регистрация Azure Stack в Azure
titleSuffix: Azure Stack
description: Узнайте, как регистрировать интегрированные системы Azure Stack в Azure, чтобы скачивать элементы Azure Marketplace и настраивать отчеты по данным.
...
ms.locfileid: "74465443"
---
# <a name="register-azure-stack-with-azure"></a>Регистрация Azure Stack в Azure

Зарегистрируйте Azure Stack в Azure, чтобы скачивать элементы Azure Marketplace из Azure и настраивать передачу коммерческих данных в корпорацию Майкрософт. После регистрации Azure Stack данные об использовании отсылаются в отдел коммерческих предложений Azure и их можно просматривать в области под идентификатором подписки для выставления счетов, использовавшегося для регистрации.
...
- установить PowerShell для Azure Stack;
- скачать средства Azure Stack;
...
Пользователь, регистрирующий Azure Stack, является владельцем субъекта-службы в Azure AD. Только пользователь, зарегистрировавший Azure Stack, может изменить регистрацию этой службы. Если пользователь без прав администратора, который не является владельцем службы регистрации, пытается зарегистрироваться (или повторно зарегистрироваться) в Azure Stack, может возникнуть ответ 403. Он указывает, что у пользователя недостаточно разрешений для выполнения операции.
...
> [!NOTE]
> Если у вас есть несколько экземпляров Azure Stack, мы советуем зарегистрировать каждый такой экземпляр в собственную подписку. Это позволяет упростить отслеживание использования.
...
Перед регистрацией в Azure удалите все имеющиеся версии средств Azure Stack и [загрузите новейшую версию с сайта GitHub](azure-stack-powershell-download.md).
...
- **Отключено**  
 С помощью развертывания Azure без подключения можно развернуть и использовать Azure Stack без подключения к Интернету. Но при этом вы будете ограничены хранилищем удостоверений службы федерации Active Directory (AD FS) и моделью выставления счетов на основе емкости.
  - [Регистрация отключенного Azure Stack с использованием модели выставления счетов **с оплатой на основе емкости**](#register-disconnected-with-capacity-billing).
...
6. Затем в том же сеансе PowerShell убедитесь, что вы вошли в правильный контекст Azure PowerShell. Это будет учетная запись Azure, которая использовалась для регистрации указанного ранее поставщика ресурсов Azure Stack. Выполните эти команды PowerShell:
...
Если вы регистрируете Azure Stack в отключенной среде (без подключения к Интернету), необходимо получить токен регистрации из среды Azure Stack. Затем используйте этот токен на компьютере, который может подключаться к Azure и на котором установлен PowerShell для Azure Stack.  
...
3. Выберите **Свойства**. В этой колонке отображаются данные о состоянии и сведения о вашей среде. Состояние может иметь значение **Зарегистрировано**, **Не зарегистрировано** или **Просрочено**.
    ...
    [![Плитка Region Management (Управление регионами) на портале администрирования Azure Stack](media/azure-stack-registration/admin1sm.png "Плитка управления регионами")](media/azure-stack-registration/admin1.png#lightbox)
...
4. Вы можете просмотреть ресурсы регистрации Azure Stack и проверить ее выполнение на портале Azure. Войдите на [портал Azure](https://portal.azure.com) с учетной записью, которая связана с подпиской, используемой для регистрации Azure Stack. Щелкните **Все ресурсы**, установите флажок **Показать скрытые типы** и выберите имя регистрации.
...
> [!NOTE]
> По завершении регистрации активное предупреждение об отсутствии регистрации больше не будет отображаться. В выпусках Azure Stack до 1904 при автономной работе в колонке управления Marketplace вы увидите запрос на регистрацию и активацию Azure Stack, даже если регистрация была выполнена. Это сообщение не отображается в выпусках после 1904.
