...
В среде комплекта разработки пользователю, если у него нет доступа к узлу комплекта разработки через удаленный рабочий стол, необходимо настроить VPN-подключение, прежде чем получать доступ к Azure Stack. См. статью [Подключение к Azure Stack](../asdk/asdk-connect.md). 
