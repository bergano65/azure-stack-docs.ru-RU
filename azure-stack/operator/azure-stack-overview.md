...
Azure предоставляет полнофункциональную платформу для разработчиков, позволяющую создавать современные приложения. К сожалению, некоторые облачные приложения сталкиваются с определенными проблемами, например с задержками, прерываниями связи и строгими требованиями. Azure в сочетании с Azure Stack позволяет реализовать следующие новые гибридные сценарии для клиентских и внутренних бизнес-приложений:
...
Azure Stack использует Azure Active Directory (Azure AD) или службы федерации Active Directory (AD FS) для предоставления удостоверений. Azure AD — это облачный, мультитенантный поставщик удостоверений Майкрософт. В большинстве гибридные сценариев с подключенным к Интернету развертыванием в качестве хранилища идентификаторов используется AAD. 

Вы можете использовать службы федерации Active Directory (AD FS) для развертываний Azure Stack без подключения. Поставщики ресурсов Azure Stack и другие приложения одинаково работают с AD FS и Azure AD. Azure Stack включает собственный экземпляр Active Directory и API Graph Active Directory.

> [!IMPORTANT]
> Поставщик удостоверений нельзя изменить после развертывания. Чтобы использовать другой поставщик удостоверений, нужно повторно развернуть Azure Stack.
...
На портале администрирования вы можете [настроить Azure Stack для доставки служб](service-plan-offer-subscription-overview.md) арендаторам, создавая и изменяя планы, квоты, предложения и подписки. Пользователи арендаторов могут подписаться на несколько предложений. В предложении может быть один или несколько планов, а в плане может быть одна или несколько служб. Операторы также управляют емкостью и реагируют на оповещения. 
