---
title: Проверка резервной копии Azure Stack с помощью ASDK
...
|IP-адрес сервера времени|Допустимый IP-адрес сервера времени, например 132.163.97.2, необходим для развертывания Azure Stack.|
|Пароль внешнего сертификата|Пароль внешнего сертификата, используемого Azure Stack. Резервная копия ЦС содержит внешние сертификаты, которые необходимо восстановить с помощью этого пароля.|
|Пароль расшифровки сертификата|Необязательный параметр. Требуется, только если резервная копия зашифрована с помощью сертификата. Пароль требуется для самозаверяющего сертификата (PFX), содержащего закрытый ключ, необходимый для расшифровки данных резервного копирования.|
|Ключ шифрования резервной копии|Необязательный параметр. Требуется, если параметры резервного копирования по-прежнему настроены с помощью ключа шифрования. Установщик будет поддерживать ключ шифрования в режиме обратной совместимости по крайней мере в течение 3 следующих выпусков. После обновления параметров резервного копирования, чтобы использовать сертификат, необходимо указать пароль для расшифровки сертификата.|
|     |     | 
