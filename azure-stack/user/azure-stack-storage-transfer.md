...
* Для обновления 1811 и более поздней версии [скачайте AzCopy V10+](/azure/storage/common/storage-use-azcopy-v10#download-azcopy).
* Для предыдущих версий (1802–1809) [скачайте AzCopy 7.1.0](https://aka.ms/azcopyforazurestack20170417).

### <a name="accopy-101-configuration-and-limits"></a>Настройки и ограничения AzCopy 10.1
...
Чтобы для AzCopy настроить версию API, которая поддерживает Azure Stack, установите значение переменной среды `AZCOPY_DEFAULT_SERVICE_API_VERSION` на `2017-11-09`.
...
|Синхронизация файла|Синхронизация контейнера с локальной файловой системой<br>Синхронизация локальной файловой системы с контейнером
   ...
   > * Azure Stack не поддерживает синхронную передачу данных между двумя расположениями больших двоичных объектов Azure Stack, а также между хранилищем Azure и Azure Stack. Команду "azcopy cp" нельзя использовать для перемещения данных из Azure Stack в хранилище Azure напрямую (или в обратном порядке) с помощью AzCopy 10.1.
...
### <a name="azcopy-known-issues"></a>Известные проблемы при использовании AzCopy
 ...
 - Если требуется перенести данные между двумя расположениями больших двоичных объектов Azure Stack или между Azure Stack и хранилищем Azure с помощью AzCopy 10.1, то сначала необходимо загрузить данные в локальное расположение, а затем повторно загрузить его в целевой каталог Azure Stack или хранилища Azure. Или можно использовать AzCopy 7.1 и указать передачу с помощью параметра **/SyncCopy**, чтобы копировать данные.  
