...
Если портал Azure Stack не предоставляет достаточно сведений для устранения неполадок или сбоя развертывания, изучите журналы кластера. Чтобы вручную извлечь журналы развертывания, обычно необходимо подключиться к одной из главных виртуальных машин кластера. Более простой альтернативный подход заключается в загрузке и запуске приведенного ниже [сценария Bash](https://aka.ms/AzsK8sLogCollectorScript), предоставленного командой Azure Stack. Этот сценарий подключается к виртуальным машинам кластера и DVM, собирает соответствующие журналы системы и кластера и загружает их обратно на рабочую станцию.
