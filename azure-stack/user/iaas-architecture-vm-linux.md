---
title: Запуск виртуальной машины Linux в Azure Stack | Документация Майкрософт
description: Узнайте, как запустить виртуальную машину Linux в Azure Stack.
...
ms.locfileid: "74993819"
---
# <a name="run-a-linux-virtual-machine-on-azure-stack"></a>Запуск виртуальной машины Linux в Azure Stack

Для подготовки к работе виртуальной машины в Azure Stack, как и в Azure, помимо самой виртуальной машины, требуются некоторые дополнительные компоненты, такие как сеть и хранилище ресурсов. Эта статья содержит рекомендации по запуску виртуальных машин Linux в Azure Stack.

![Архитектура для виртуальной машины Linux в Azure Stack](./media/iaas-architecture-vm-linux/image1.png)
...
Виртуальную машину можно подготовить на основе списка опубликованных образов, пользовательского управляемого образа или файла виртуального жесткого диска (VHD), передав его в хранилище BLOB-объектов Azure Stack. Azure Stack поддерживает ряд популярных дистрибутивов Linux, включая CentOS, Debian, Red Hat Enterprise, Ubuntu и SUSE. Дополнительные сведения см. в статье [Добавление образов Linux в Azure Stack Marketplace](https://docs.microsoft.com/azure-stack/operator/azure-stack-linux). Вы также можете выполнить синдикацию одного из опубликованных образов Linux, доступных в Azure Stack Marketplace.
...
**Доступность** Виртуальная машина может перезагружаться из-за планового обслуживания, запланированного оператором Azure Stack. Для повышения уровня доступности разверните несколько виртуальных машин в [группе доступности](https://docs.microsoft.com/azure-stack/operator/app-service-deploy-ha).

**Резервные копии**. Рекомендации по защите виртуальных машин IaaS Azure Stack см. в [этой](https://docs.microsoft.com/azure-stack/user/azure-stack-manage-vm-protect) статье.
...
**Шифрование данных**. Azure Stack защищает данные инфраструктуры и пользователя на уровне подсистемы хранения с помощью шифрования неактивных данных. Подсистема хранения Azure Stack шифруется с помощью 128-битного шифрования AES BitLocker. Дополнительные сведения см. в [этой](https://docs.microsoft.com/azure-stack/operator/azure-stack-security-bitlocker) статье.
