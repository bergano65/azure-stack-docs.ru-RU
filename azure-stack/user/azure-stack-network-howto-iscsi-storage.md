...
Шаблон группы ресурсов создает выходные данные, которые следует использовать в качестве входных данных для следующего шага. Главным образом определяется имя сервера и общедоступный IP-адрес Azure Stack, в котором инициируется трафик iSCSI. В нашем примере:
...
|**Параметры**|**значение по умолчанию**|**description**|
|------------------|---------------|------------------------------|
...
|VNetName                |Хранилище           |Имя виртуальной сети. Оно будет использоваться для обозначения ресурсов
|VNetAddressSpace        |10.10.0.0/23      |Диапазон адресов для виртуальной сети
|VNetInternalSubnetName  |Внутренний          |Имя внутренней подсети виртуальной сети
...
|**Входные данные**|**значение по умолчанию**|**description**|
|------------------|---------------|------------------------------|
...
|ChapUsername         |Имя пользователя                 |Имя пользователя для проверки подлинности CHAP
