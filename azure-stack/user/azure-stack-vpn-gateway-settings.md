---
title: Настройка параметров VPN-шлюза для Azure Stack | Документация Майкрософт
description: Узнайте о VPN-шлюзах, используемых для Azure Stack, и настройке их параметров.
...
Аналогичным образом Azure Stack не поддерживает изменение номера SKU для поддерживаемых устаревших ценовых категорий (**Базовый**, **Стандартный** и **HighPerformance**) на новые номера SKU, которые поддерживаются в Azure (**VpnGw1**, **VpnGw2** и **VpnGw3**).
...
> [!IMPORTANT]  
> Сейчас Azure Stack поддерживает только VPN на основе маршрутов. Если устройство поддерживает только VPN на основе политик, подключения к этим устройствам из Azure Stack не поддерживаются.  
>
> Кроме того, сейчас Azure Stack не поддерживает использование селекторов трафика на основе политик для шлюзов на основе маршрутов, так как настраиваемые конфигурации политики IPSec/IKE не поддерживаются.
...
>[!IMPORTANT]
>Чтобы подсети шлюзов работали правильно, каждую подсеть нужно назвать **GatewaySubnet** . Azure Stack использует это имя, чтобы определить подсеть для развертывания виртуальных машин и служб шлюза виртуальной сети.
...
При настройке VPN-подключения в Azure Stack оно должно быть настроено на обеих сторонах. Возможно, при настройке VPN-подключения между Azure Stack и устройством, например коммутатором или маршрутизатором, выполняющем функции VPN-шлюза, вам потребуется настроить дополнительные параметры для устройства.

В отличие от Azure, где поддерживается несколько предложений (как инициатора, так и отвечающего устройства), Azure Stack по умолчанию поддерживает только одно предложение.  Если вам нужно использовать разные параметры IPSec/IKE для работы с VPN-устройством, доступны дополнительные параметры для настройки подключения вручную.  Дополнительные сведения см. в статье [Настройка политики IPsec/IKE для VPN-подключений типа "сеть — сеть" или "виртуальная сеть — виртуальная сеть"](azure-stack-vpn-s2s.md).
