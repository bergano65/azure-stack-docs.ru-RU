...
В пакете SDK для Go профили доступны в пути к профилям с версией в формате **ГГГГ-ММ-ДД**. **2019-03-01** — последняя версия профиля API Azure Stack версии 1904 или более поздней на данный момент. Чтобы импортировать данную службу из профиля, импортируйте из профиля ее соответствующий модуль. Например, чтобы импортировать службу **вычислений** из профиля версии **2019-03-01**, используйте следующий код:
...
1. Если в подписке доступен субъект-служба с ролью владельца, пропустите этот шаг. В противном случае инструкции по созданию субъекта-службы, которая использует секрет клиента, и назначению ей роли "Владелец" в пределах вашей подписки см. в статье [Предоставление приложениям доступа к Azure Stack](../operator/azure-stack-create-service-principals.md). Обязательно сохраните идентификатор приложения и секрет субъекта-службы.
...
- [Использование управляемых дисков ](https://github.com/Azure-Samples/Hybrid-Compute-Go-ManagedDisks)(пример, использующий версию 2019-03-01-hybrid, которая предназначена для последних версий API, поддерживаемых Azure Stack).
