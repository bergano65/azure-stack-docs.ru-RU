...
- Интегрированная система Azure Stack или Пакет средств разработки Azure Stack (ASDK), развернутые и настроенные в соответствии со следующими инструкциями. 
  ...
  1. Создайте [план или предложение](../operator/service-plan-offer-subscription-overview.md) в Azure Stack.
...
1. Выберите репозиторий для кода. Azure Pipelines анализирует проект и идентифицирует его тип как ASP.NET Core, поэтому открывает стандартный шаблон сборки для ASP.NET Core *azure-pipelines.yml*. 
   
   ![Файл azure-pipelines.yml для ASP.NET Core](media/azure-stack-solution-pipeline/buildargument.png)
   
1. Вы можете напрямую править код конвейера или выбрать команду **Показать помощника**, которая открывает панель **Задачи** для добавления задач и шагов. 
   
   Если вы выбрали вариант **Показать помощника**, щелкните **.NET Core** на панели **Задачи**. В форме **.NET Core** выполните следующее.
   - Раскройте список **Команда** и выберите **publish** (опубликовать). 
   - В поле **Аргументы** введите *-r win10-x64*.
   - Убедитесь, что установлен флажок **Публикация веб-проектов**.
   - Выберите **Добавить**.
   
   Вместо использования помощника вы можете напрямую править файл *azure-pipelines.yml*, добавив в него следующий код:
   
   - В области `pool` измените для параметра `vmImage` значение `ubuntu-latest` на `vs2017-win2016`.
     
   - В области `steps` добавьте задачу [DotNetCoreCLI](/azure/devops/pipelines/tasks/build/dotnet-core-cli) с командой и аргументами: 
     ...
     ```
   Теперь файл *azure-pipelines.yml* будет содержать следующий код: 
   ...
   - task: DotNetCoreCLI@2
     inputs:
       ...
       arguments: '-r win10-x64'
   ```
1. Щелкните **Сохранение и запуск**, добавьте сообщение о фиксации и описание (необязательно), и снова щелкните **Сохранение и запуск**. 
   
Процесс [сборки автономного развертывания](/dotnet/core/deploying/#self-contained-deployments-scd) публикует артефакты, которые могут выполняться в Azure и Azure Stack.
...
1. На странице **Выбор шаблона** выберите **Развертывание Службы приложений Azure** и щелкните **Применить**.
...
1. В области слева выберите **Запуск в агенте**. В области справа выберите **Azure Pipelines** из раскрывающегося списка **Пул агентов** , а затем выберите **vs2017-win2016** из раскрывающегося списка **Спецификация агента**.
   
   ![Выбор размещенного агента](media/azure-stack-solution-pipeline/agentjob.png)
   
1. В области слева выберите **Развернуть Службу приложений Azure**. Прокрутите вниз область справа и нажмите значок многоточия ( **...** ) рядом с элементом **Пакет ии папка**.
   
   ![Выбор пакета или папки](media/azure-stack-solution-pipeline/packageorfolder.png)
   
1. В диалоговом окне **Выберите файл или папку** перейдите к расположению сборки веб-приложения Azure и щелкните **OK**.
   
1. На странице **Создать конвейер выпуска** щелкните **Сохранить** в правом верхнем углу. 
...
1. В разделе **Этапы** наведите указатель мыши на этап **Azure**, дождитесь появления **+** и щелкните **Добавить**.
   
1. В поле **Шаблон** выберите вариант **Пустое задание**. 
   
1. В диалоговом окне **Этап** ведите имя этапа *Azure Stack*. 
...
Так как в конвейере выпуска задан триггер непрерывного развертывания, изменение исходного кода автоматически инициирует новую сборку и создание нового выпуска. Но на этот раз вы создадите и запустите новый выпуск вручную.
