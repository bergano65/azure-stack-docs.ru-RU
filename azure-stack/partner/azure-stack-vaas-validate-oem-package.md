...
6. Скопируйте папку пакета обновления Azure Stack в локальный каталог на виртуальной машине для разработки. Для параметра Azure Stack update package folder path (Путь к папке пакета обновления Azure Stack) введите путь к родительскому каталогу.

7. Скопируйте созданную ранее папку пакета OEM в локальный каталог на виртуальной машине для разработки. Для параметра OEM update package folder path (Путь к папке пакета обновления OEM) введите путь к родительскому каталогу.

    > [!NOTE]
    > Копируйте пакеты обновлений Azure Stack и OEM в **два отдельных** родительских каталога.
...
Для проверки пакетов OEM необходимо выполнить следующие тесты в указанном порядке.
...
2. Для проверки пакета нужно выполнить **необходимые тесты** в указанном порядке.

    > [!CAUTION]
    > VaaS выполняет тесты в том порядке, в котором они были запланированы. Тесты необходимо запланировать в указанном порядке.

3. Выберите агент, с помощью которого будет выполняться тест. Дополнительные сведения о добавлении локальных агентов для тестирования см. в статье [Развертывание локального агента](azure-stack-vaas-local-agent.md).

4. Для планирования запуска теста в контекстном меню выберите **Расписание**, чтобы открыть командную строку для планирования запуска тестового экземпляра.

5. Проверьте параметры теста, а затем выберите **Отправить**, чтобы запланировать тест.

6. Чтобы запланировать запуск следующего теста, не нужно ждать завершения предыдущего теста. Запланируйте все **необходимые** тесты в указанном выше порядке.

7. Проверьте результаты **необходимых** тестов.
