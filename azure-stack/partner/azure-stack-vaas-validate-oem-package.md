...
6. Скопируйте папку пакета обновления Azure Stack в локальный каталог на виртуальной машине для разработки. В поле AzureStack update package folder path (Путь к папке пакета обновления AzureStack) введите путь к **папке, содержащей ZIP-файл пакета и файл метаданных**.
